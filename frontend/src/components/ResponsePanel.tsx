...
const ResponsePanel = ({ isLoading, error, response }: ResponsePanelProps) => {
	return (
		<div className="rounded-3xl border border-white/10 bg-white/5 p-5">
			...
			<div className="mt-3 min-h-30 rounded-2xl border border-white/10 bg-black/40 p-4 text-xs text-white/70">
				...
				{response && (
					<pre className="whitespace-pre-wrap wrap-break-words overflow-auto">{JSON.stringify(response, null, 2)}</pre>
				)}
			</div>
		</div>
	);
